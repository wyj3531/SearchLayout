...
public class SearchHelper {
    ...
    private static final String DATABASE_CREATE =
            "CREATE VIRTUAL TABLE " + FTS_VIRTUAL_TABLE + " USING fts3("
                    + COLUMN_NAME + " UNIQUE (" + COLUMN_NAME + "));";
    ...
    public long createList(List<String> names) {
        ContentValues values = new ContentValues();
        for (String name : names) {
            values.put(COLUMN_NAME, name);
        }
        return mDb.insert(FTS_VIRTUAL_TABLE, null, values);
    }

    public long createName(String name) {
    ...
    public Cursor queryAllNames() {
        Cursor cursor = mDb.query(FTS_VIRTUAL_TABLE, null, null, null, null, null, null);
